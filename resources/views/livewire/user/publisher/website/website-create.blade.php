...
<x-jet-dialog-modal wire:model="confirmingWebsiteAdd">
    ...
    <x-slot name="content">
        ...
        <div class="col-span-6 sm:col-span-4 mb-2">
            <x-jet-label for="name" value="{{ __('Website Domain') }}" />
            <div class="flex">
            <select name="protocol" class="flex-shrink-0 inline-flex items-center bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-l-lg focus:ring focus:ring-indigo-200 focus:border-indigo-300 focus:ring-opacity-50 w-1/4 px-3 py-2 mt-1 dark:bg-gray-800 dark:border-gray-700 dark:placeholder-gray-500 dark:text-gray-200" wire:model.defer="protocol" required>
                ...
                <option value="{{$key}}" wire:key="{{$key}}" {{$key === $protocol? 'selected': ''}}>{{$secure}}</option>
                @endforeach
            </select>
            <x-jet-input id="name" type="text" class="mt-1 w-3/4" rounded="rounded-r-lg" wire:model.defer="domain_name" autocomplete="domain_name" required/>
            </div>
            <x-jet-input-error for="domain_name" class="mt-2" />
            <x-jet-input-error for="protocols" class="mt-2" />
        </div>
            <!-- Monthly Traffic -->
            <div class="col-span-6 sm:col-span-4 mb-2">
                <x-jet-label for="traffic" value="{{ __('Monthly Traffic') }}" />
                <x-jet-input id="traffic" type="number" min="5000" class="mt-1 block w-full" wire:model.defer="monthly_traffic" autocomplete="monthly_traffic" required />
